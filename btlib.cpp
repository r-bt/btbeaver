...
#include "btbeaver/received_packet_types.h"

#define AUTO_RF 1
#define AUTO_DIS 2
#define AUTO_MSC 3

#include "btbeaver/sendhci_packets.h"
...
#include "btbeaver/uuids.h"

#include "btbeaver/hci_errors.h"
...
int init_blue_ex(char* filename, int hcin)
{
  ...
  while (readret > 0 && errcount == 0) {
    ...
    if (flag != 0) {
      ...
      if (flag == 1) {
        if (gpar.btleflag != 0 || devnfrombadd(dev[dn]->baddr, BTYPE_XALL, DIRN_FOR) == 0) { // is device 0 board address local - move to dn=0
...
struct cticdata* cticalloc(int ndevice)
{
  ...
  if ((*cpp)->type == CTIC_END) {
    ...
    cp = static_cast<cticdata*>(malloc(sizeof(struct cticdata)));
    if (cp == NULL) {
      NPRINT "Alloc error\n");
      return (&cticnull);
    }
...
int devalloc()
{
  ...
  if (dev[dn] == NULL) {
    dev[dn] = static_cast<devdata*>(malloc(sizeof(struct devdata)));
    if (dev[dn] == NULL) {
      NPRINT "Alloc error\n");
      return (-1);
    }
  ...
  do {
    ...
    for (repn = 0; repn < insdat[n + 1]; ++repn) { // each response
      ...
      { // each data entry
        ...
        if (rp[j + 1] == 0xFF && (rp[j + 2] == ((rp[2] ^ rp[3]) ^ rp[4])) && (rp[j + 3] == (((rp[5] ^ rp[6]) ^ rp[7]) | 0xC0))) {
...
void mesh_server(int (*callback)(int clientnode, char* buf, int nread))
{
...
int notify_ctic(int node, int cticn, int notifyflag, int (*callback)(int node, int cticn, unsigned char* data, int dataSize))
{
...
int writecticx(int node, int cticn, unsigned char* data, int count, int notflag, int (*callback)(int node, int cticn, unsigned char* data, int dataSize))
{
  ...
  if (ndevice == 0) { // local
    ...
    for (devn = 1; cp->notify != 0 && devok(devn) != 0; ++devn) { // search all other devices for notify
      dp = dev[devn];
      if ((dp->conflag & CON_LX) != 0 && (gpar.btleflag == 0 || gpar.btlenode == 0 || gpar.btlenode == dp->node)) { // device devn is connected as LE client
                                                                                                                    // send notification
...
int readhci(int ndevice, long long int mustflag, long long int lookflag, int timout, int toshort)
{
  ...
  do {
    ...
    do // wait for complete message
    {
      ...
      if (blen < wantlen) // need more or short TO to exit
      {
        do // read block of data - may be less than or more than one line
        {
          ...
          if (len <= 0 && (timendms == 0 || (timems(TIM_RUN) - timstart) >= timendms)) { // nothing read and timed out - normal exit route
            if (len > 0 || blen > 0)
    ...
    if (buf[0] == 4) // HCI events
    {
      ...
      if ((n0 & 0x80) != 0) { // handle at n0
        ...
        for (k = 1; devicen == 0 && devok(k) != 0; ++k) {
          if (dev[k]->type && (BTYPE_CL | BTYPE_LE | BTYPE_ME) != 0 && dev[k]->conflag != 0) { // hi 4 bits of buf[2] = flags
      ...
      if ((buf[2] & 0x30) == 0x10) { // extra data
        ...
        if (chan == 4) // LE or mesh
        {
          if ((dev[devicen]->conflag & CON_LE) != 0 && (buf[9] == 0x1B || buf[9] == 0x1D)) { // client notify
            ...
            if ((dev[devicen]->conflag & CON_LE) != 0 && (buf[9] & 1) == 0 && buf[9] <= 0x20) { // even opcode = request from server - let odd opcodes go to ATTDAT
              gotflag = 0; // ditch
              if (buf[9] == 0x02) { // MTU exhange
                ...
                else if ((buf[9] == 0x08 || buf[9] == 0x10) && buf[10] == 0x01 && buf[11] == 0x00 && buf[14] == 0x00 && buf[15] == 0x28) {
                  if (buf[9] == 0x08)
        ...
        } else if (chan >= 0x40) // dynamic channel
        {
          ...
          if ((sflag & IN_SSAREP) != 0 && (buf[9] == 1 || buf[9] == 3 || buf[9] == 5 || buf[9] == 7) && buf[10] == 0)
            ...
            gotflag = IN_PNRSP;
          else if (buf[10] == 0xEF && buf[9] == 0x03 && (buf[12] == 0xE1 || buf[12] == 0xE3 || buf[12] == 0x93 || buf[12] == 0x91)) { // copy for reply
    ...
    if (gpar.printflag == PRINT_VERBOSE && stopverb == 0) { // only print le scan reply gotflag == LE_SCAN or meshflag = 1
      //  igflag = 0;
      b0 = buf[0];
    ...
    if ((xflag == 0 && doneflag == 0 && mustflag != 0 && locmustflag == 0) || (disflag != 0 && ndevice == devicen)) { // done - switch to short timeout
...
void immediate(long long lookflag)
{
  ...
  while (1) {
    ...
    if (gotflag == IN_ATTDAT && (dp->conflag & CON_LX) != 0) {
      ...
      if ((dp->psm == 1 && (dp->conflag & CON_PSM1X) == 0) || (dp->psm == 3 && (dp->conflag & CON_PSM3X) == 0)) { // disconnect request initiated by remote
        dp->id = 11;
      ...
      psm = insdat[n + 4];
      if ((dp->conflag & CON_SERVER) == 0 || !((psm == 1 && (dp->conflag & CON_PSM1) == 0) || (psm == 3 && (dp->conflag & CON_PSM3) == 0))) { // only allow server psm 1/3
  ...
  if (dat[0] == 0x04) {
    ...
    do {
      ...
      if (cticn >= 0) {
        ...
        psn = cticn & 0xFFFF;
        if (psflag != 0) {
          if (dat[5] == 0x00 && dat[6] == 0x28 && ((datcount == 2 && pserv[psn].uuidtype == 2 && dat[7] == pserv[psn].uuid[1] && dat[8] == pserv[psn].uuid[0]) || (datcount == 16 && pserv[psn].uuidtype == 16 && bincmp(pserv[psn].uuid, dat + 7, 16, DIRN_REV) != 0))) {
            s[10] = handle & 0xFF;
            s[11] = (handle >> 8) & 0xFF;
            eog = pserv[psn].eog;
            s[12] = eog & 0xFF;
      ...
      do {
        ...
        if (cticn >= 0) {
          ...
          if (psflag != 0) {
            ...
            if (handle == cp->chandle - 1 && (aflag == 3 || (uuidtype == 2 && dat[5] == 0x03 && dat[6] == 0x28))) { // info 2803
              ...
              flag = 1;
            } else if (handle == cp->chandle + 1 && aflag == 0 && uuidtype == 2 && dat[5] == 0x02 && dat[6] == 0x29) { // notify control 2902
              size = 2;
              if (dn != 0) { // insert end of group handle
              ...
              s[14 + dn] = 0;
              flag = 1;
            } else if (handle == cp->chandle && aflag == 0 && uuidtype == cp->uuidtype && bincmp(cp->uuid, dat + 5, cp->uuidtype, DIRN_REV) != 0) { // value
              size = cp->size;
              if (dn != 0) { // insert end of group handle
                s[13] = pserv[cp->psnx & 0xFFFF].eog & 0xFF;
                s[14] = (pserv[cp->psnx & 0xFFFF].eog >> 8) & 0xFF;
...
void rwlinkey(int rwflag, int ndevice)
{
  ...
  if (rwflag == 0) { // read
    ...
    for (k = 0; k < count; ++k) {
      ...
      if ((ndevice == 0 && n > 0) || (ndevice > 0 && n == ndevice)) { // all on init (ndevice=0)  or new classic ndevice only
        dp = dev[n];
        for (n = 0; n < 16; ++n)
          dp->linkey[n] = key[n];
...
int meshpacket(unsigned char* s)
{
  ...
  for (repn = 0; repn < s[4]; ++repn) { // each response
    ...
    if (rp[10] == 0xFF && (rp[11] == ((rp[2] ^ rp[3]) ^ rp[4])) && (rp[12] == (((rp[5] ^ rp[6]) ^ rp[7]) | 0xC0))) { // is a mesh packet
      index = rp[13] + (rp[14] << 8);
...
int findhci(long long int type, int devicen, int popflag)
{
  ...
  n = 0;
  while (instack[n] != INS_FREE) {
    if (instack[n] != INS_POP && (type & ((long long int)1 << (instack[n] & 0x3F))) != 0 && (devicen == 0 || devicen == instack[n + 3])) {
      if (popflag == INS_POP)
...
int clservices(int ndevice, int flags, char* uuid)
{
  ...
  if (sn > 0) {
    ...
    if (flags == SRVC_FINDUUID2 || flags == SRVC_FINDUUID16) { // search channels for 16-byte or 2-byte RFCOMM UUID match
      for (k = 0; serv[k].channel != 0 && k < SERVDAT; ++k) {
        if ((flags == SRVC_FINDUUID16 && serv[k].uuidtype == 16) || (flags == SRVC_FINDUUID2 && serv[k].uuidtype == 2)) {
          getout = 0;
...
int list_ctics(int node, int flag)
{
  ...
  if (!(dev[ndevice]->type == BTYPE_LE || dev[ndevice]->type == BTYPE_ME || (ndevice == 0 && ctic(0, 0)->type == CTIC_ACTIVE))) {
...
int printctics0(int devicen, int flags)
{
  ...
  for (n = 0; ctic(devicen, n)->type == CTIC_ACTIVE; ++n) {
    ...
    if ((flags & (CTIC_R | CTIC_W | CTIC_NOTIFY)) == 0 || (perm == 0 && (flags & CTIC_NOTIFY) == 0) || ((flags & CTIC_NOTIFY) != 0 && (perm & 0x30) != 0) || ((flags & CTIC_R) != 0 && (perm & 0x02) != 0) || ((flags & CTIC_W) != 0 && (perm & 0x0C) != 0)) {
...
int setlelen(int ndevice, int len, int flag)
{
  ...
  if (xlen > dev[ndevice]->setdatlen && flag != 0 && !(dev[ndevice]->type == BTYPE_ME && (dev[ndevice]->conflag & CON_LE) == 0)) { // not for node connect
    VPRINT "Set MTU\n");
...
int decodedes(unsigned char* sin, int len, struct sdpdata* sdpp) //  int level,int aidflag,int record,unsigned int callaid)
{
  ...
  do {
    ...
    } else // single value - not a des
    {
      ...
      if (sdpp->aid == 0x0100 && (type == 4 || type == 8) && sdpp->chdn >= 0 && sdpp->nameflag != 0) { // aid = 0100 service name text
        j = 0;
    ...
    do // until no more IN_DATA on stack
    {
      ...
      if (n >= 0) { // data on instack - may be on first entry
        ...
        if (devicen == 0) {
           ...
           if ((devok(ndev) != 0) && ((((*ndevice & FROM_MESH) != 0) && (dev[ndev]->type == BTYPE_ME)) || (((*ndevice & FROM_CLCON) != 0) && (dev[ndev]->conflag & CON_RF) != 0) || (((*ndevice & FROM_MESHCON) != 0) && (dev[ndev]->conflag & CON_MESH) != 0))) { // read from this device
            devicen = ndev;
        ...
        do // while still something on instack
        {
          ...
          if (meshflag == 0 && (((flag & EXIT_CHAR) != 0 && lastchar == endchar) || ((flag & EXIT_COUNT) != 0 && gotn == count))) {
            popins();
            gpar.readerror = 0; // OK
            timems(TIM_FREE);
...
void flushprint()
{
  ...
  if (gpar.prtp > PRBUFSZ - PRPAGESZ || (gpar.prtw != 0 && gpar.prtp >= gpar.prtw)) { // gone past limit or existing wrap - wrap to start of buffer - leave prte
